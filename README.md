...
A **"Why these numbers?"** explainer on the Recommend screen turns the whole thing into plain
sentences (base, what each choice moved, physics-vs-linear, confidence + what to do about it).
It's a deterministic template — **free, offline, no LLM, no key** — because the app already
computes every part; it just needs wording. An optional LLM chat for *open-ended* questions is
the paid/free-tier upgrade path (see roadmap), but the explainer covers "why these numbers".
